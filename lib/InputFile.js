...
function parseParts(inputFile, text, offset = 1) {
  ...
  if (!layoutHasContent(layout)) {
    let errorOffset = spec.lines.length + offset
    return inputFile.userError('Layout section must have content', errorOffset)
  }
...
function layoutHasContent(numberedLines) {
  let contentFound = false
  for (var i = 0; i < numberedLines.lines.length; i++) {
    if (numberedLines.lines[i].string.search(/\S/) != -1) {
      contentFound = true
      break
    }
  }

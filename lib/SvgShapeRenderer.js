...
module.exports = class SvgShapeRenderer {
  ...
  constructor(context, styles, outputPath, debug) {
  ...
  renderShapeToSvg(shape, diagramShape = false) {
    ...
    for (var layer of shape.layers) {
      ...
      let {buf, transforms} = this.renderLayer(layer, shape.margin, this.context, diagramShape, e)
  ...
  renderLayer(layer, margin, context, diagramShape, e) {
    const {grid, rowHeights, colWidths} = layer
    const makeDebugGrid = layer.index === 0
                          && ((this.debug.showGrid == true
                              && diagramShape)
                              ||
                              (this.debug.showShapeGrid == true
                               && !diagramShape))
  ...
  makeStyleBlock(obj) {
    ...
    if (style != null) {
      ...
      styleData.classes.push(style.name)
      styleData.classStyle[style.name] = this.getStyleDataForClass(style.name)

      if (style.plus != null && style.plus.length > 0) {
        styleBlock += ` style="${style.plus}"`

        styleData.style += '; ' + style.plus
      }
    }
    return {
      styleBlock,
  ...
  renderShape(obj, sizeAndPosition, styleBlock, svgBlock) {
    ...
    const shapeRenderer = new SvgShapeRenderer(this.context, this.styles, this.outputPath, this.debug)

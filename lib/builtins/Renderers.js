...
function renderText(obj, sizeAndPosition, styleBlock) {
  ...
  styleBlock = applyTextAlignment(gridAlign, styleBlock)
...
function renderPath(obj, sizeAndPosition, styleBlock, svgBlock) {
  const {cx, cy} = sizeAndPosition
...
function applyTextAlignment(gridAlign, styleBlock) {
  ...
  const reClass = /class="([^"]+)"/
  const reStyle = /(style="[^"]+")/
  const classMatch = styleBlock.match(reClass)
  const styleMatch = styleBlock.match(reStyle)
  let part1 = ''
  let part2 = ''
  if (classMatch != null) {
    part1 = classMatch[1] + ' '
  ...
  return ` class="${part1}${alignClass}"${part2}`
}

...
module.exports = class DiagramParser {
  ...
  parseSpec(inputFile, variables = this.variables, settings = this.settings) {
    ...
    while ((line = inputFile.pop()) != null) {
      ...
      if (parser === null) {
        parser = this.findParser(line, inputFile)
        line = inputFile.pop()
      }
      const ele = parser(line, inputFile, variables, settings)
      if (ele != null) {
  ...
  parseShape(inputFile) {
    ...
    for (var i = 0; i < inputFile.lines.length; i++) {
      ...
      if (inputFile.lines[i].string.startsWith('layout:')) {
        method = 'spec'
        break
      }
    }
  ...
  findParser(line, inputFile) {
    const match = line.match(/^(\S+?):/)
    let parser = null
    if (match != null) {
      parser = this.parsers[match[1].toLowerCase()]
      if (parser === undefined) {
        inputFile.userError(`Unknown type: ${match[1]}`)
      }
    }
    if (parser == null) {
      inputFile.userError(`Unexpected: ${line}`)
    }
    return parser
  }

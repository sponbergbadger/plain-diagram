...
module.exports = class DiagramParser {
  ...
  parseDiagram(text) {
    ...
    const {spec, layout} = new InputFile(text).fileSections
    const {shapes} = this.parsePartsIntoLines(text)
  ...
  parseShape(lines) {
    ...
    if (method === 'spec') {
      const {spec, layout} = new InputFile(lines.join('\n')).fileSections

      this.parseSpec(spec, this.variables, settings)

      lines = layout.lines.reduce((acc, it) => {
        acc.push(it.string)
        return acc;
      }, []);
    } else {

...
module.exports = class DiagramParser {
  ...
  initializeParserState() {
    ...
    this.defaultStyles = {
      'text': 'font: 12px sans-serif; dominant-baseline: middle; text-anchor: middle',
      'rect': 'fill: none',
      'ellipse': 'fill: none',
    }
    this.margin = {
    ...
    this.spacer = {
      horizontal: defaultSpacer.horizontal,
      vertical: defaultSpacer.vertical,
      'text-width': defaultSpacer['text-width'],
      'text-height': defaultSpacer['text-height'],
      'grid-align': defaultSpacer['grid-align'],
      'grid-valign': defaultSpacer['grid-valign'],
    }
    this.debug = {
      showGrid: false
    }
  ...
  initializeParsers(diagramFactory) {
    ...
    this.parsers['spacer'] = (line, rem, variables, spacer) => { this.parserSpacer(line, variables, spacer) }
  ...
  parsePartsIntoLines(text) {
    ...
    spec = this.moveToTop(spec, 'spacer')
  ...
  parseSpec(lines, variables = this.variables, spacer = this.spacer) {
    let parser = null
    let line
    while ((line = lines.shift()) != null) {
      const commentI = Util.commentIndex(line)
      ...
      const ele = parser(line, lines, variables, spacer)
      if (ele != null) {
  ...
  parseShape(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
      // Only allow layout, variable, spacer
    ...
    if (method === 'spec') {
      ...
      this.parseSpec(spec, this.variables, spacer)
      lines = layout
    }
  ...
  parserDefaultStyles(line, variables) {
    ...
    if (key == 'text') {
      if (val.indexOf('dominant-baseline:') === -1) {
        var token = ''
        if (!val.endsWith(';')) {
          token = ';'
        }
        val += token + ' dominant-baseline: middle;'
      }
      if (val.indexOf('text-anchor:') === -1) {
        var token = ''
        if (!val.endsWith(';')) {
          token = ';'
        }
        val += token + ' text-anchor: middle;'
      }
    }
  ...
  parserSpacer(line, variables, spacer) {
    let {key, val} = parseKeyValue(line, variables)
    if (val === 'horizontal'
        || val === 'vertical'
        || val === 'text-width'

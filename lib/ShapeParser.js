...
module.exports = class ShapeParser {
  ...
  constructor(diagramParser, margin = zeroMargin) {
    ...
    this.spacer = diagramParser.spacer
  ...
  parseLayout(lines, params = {}, spacer = this.spacer, width = null, height = null, parentPathAngle = 0) {
    let line = lines.shift()
    let result = this.parserLayout(line, lines, params, spacer, width, height, parentPathAngle)
    if (result != null && result.layers != null) {
      return result
    }
    throw new Error('No layout found')
  }

  parserLayout(line, rem, params, spacer, fillToWidth = null, fillToHeight = null, parentPathAngle = 0) {
    const layersToParse = this.parseLayoutLines(line, rem)

    this.makeLayerIndexToZIndexMap(layersToParse)

    this.addLayers(layersToParse, params, spacer, fillToWidth, fillToHeight, parentPathAngle)
  ...
  parseLayoutLines(line, rem) {
    ...
    while ((line = rem.shift()) != null) {
      let skipThisLine = false
      if (newLayer && line.trim() === '') {
        skipThisLine = true
      }
      newLayer = false
      if (skipThisLine) {
      ...
      if (line.trim().startsWith('*')) {
        ...
        // New layer
        layoutLines = []
        layoutLineDesc = line.trim().substring(1)
        newLayer = true
      } else {
        layoutLines.push(line)
      }
  ...
  addLayers(layersToParse, params, spacer, fillToWidth, fillToHeight, parentPathAngle) {
    let layerInfo = {
      zIndex: null,
      location: null
    }
    ...
    for (var i = 0; i < layersToParse.length; i++) {
      ...
      this.addLayer(layoutLines, layerInfo, params, spacer, pathDistance, pathAngle, fillTo)
    }
  }
  ...
  makeLayerIndexToZIndexMap(layersToParse) {
    ...
    for (var i = 1; i < layersToParse.length; i++) {
      ...
      const match = lineDesc.match(reZ)
      if (match != null) {
        const zIndex = match[1]
        layerIndexToZIndexMap[i] = zIndex
        if (zIndexToLayerIndex[zIndex] != null) {
          throw new Error('Layer at z-index ${zIndex} already exists')
        }
  ...
  addLayer(layoutLines, layerInfo, params, spacer, pathDistance, pathAngle, fillTo) {
    if (layoutLines[layoutLines.length - 1].trim().length === 0) {
      // Drop the last row, but not more. More rows equals more space
      layoutLines.pop()
    }
    const colGraph = this.findColumnGraph(layoutLines)
    const layer = this.makeGrid(layoutLines, colGraph, params, spacer, pathDistance, pathAngle, fillTo, layerInfo.userFillTo)
  ...
  parseLayerInfo(line, params) {
    ...
    if (info == null) {
      throw new Error(`Invalid layer definition: ${line}`)
    }
  ...
  getLayerInfoUsingAt(line, params) {
    const {svg, rotateTo, fillTo} = parseOutSvgFillRotate(line)
    ...
    let offset = this.resolveOffset(match[8], params, line)

    let {anchorKey, anchorIndex, layerZIndex, next} = this.resolveAnchorRef(match[4], params)
  ...
  getLayerInfoUsingFromTo(line, params) {
    const {svg, rotateTo, fillTo} = parseOutSvgFillRotate(line)
    ...
    let fromOffset = this.resolveOffset(match[6], params)
    let toOffset = this.resolveOffset(match[12], params)
    let pointOffset = this.resolveOffset(match[16], params)
    let layerOffset = this.resolveOffset(match[20], params, line)

    let pathPoint = match[14]

    let fromRef = this.resolveAnchorRef(match[4])
    let toRef = this.resolveAnchorRef(match[10])
  ...
  resolveAnchorRef(anchor, params = {}) {
    ...
    if (layerZIndex == null) {
      ...
      if (this.layersByZIndex[layerZIndex] == null) {
        throw new Error(`Unknown layer index: ${layerZIndex}`)
      }
      ref = this.layersByZIndex[layerZIndex].layerObjectMap[anchorKey]
    }
    if (ref == null) {
      throw new Error(`Reference not found: ${anchor}`)
    }
  ...
  findColumnGraph(lines) {
    ...
    for (var col of colsBounds) {
      ...
      for (var line of lines) {
        let s = line.substring(col.s, col.e)
        if (s === '.') {
          hasDot = true
    ...
    // If the first column has no space, also
    //  zero it out
    col = colsBounds[0]
    let hasNonSpace = false
    for (var line of lines) {
  ...
  makeGrid(lines, colGraph, params = {}, spacer, pathDistance, pathAngle, fillTo, userFillTo) {
    const grid = [] // [rows][columns]

    // [type] = [array of instances for this type containing x,y coordinate positions]
    const layerObjectMap = {}

    const {rowHeights, colWidths} = this.initializeColumnAndRowArraysWithSpacers(lines, colGraph, spacer)
    ...
    for (var y = 0; y < lines.length; y++) {
      ...
      for (var x = 0; x < colGraph.length; x++) {
        ...
        if (!this.fillGridPosition(line, spacer, params, col, grid, x, y, layerObjectMap, shapesToFill, pathAngle)) {
    ...
    this.initializeRowSpacers(lines, colGraph, grid, rowHeights, spacer)
    ...
    this.distributeHeights(grid, rows, cols, rowHeights, fillTo.height, userFillTo.height)

    // Layout remaining fillable shapes
    this.layoutFillableShapes(grid, shapesToFill, pathAngle, true)

    // The author made an error if any of the colWidths or rowHeights are zero
    this.assertNonZeroWidthAndHeight(colWidths, rowHeights)
  ...
  initializeColumnAndRowArraysWithSpacers(lines, colGraph, spacer) {
    const rowHeights = []
    const colWidths = []
    for (var y = 0; y < lines.length; y++) {
      rowHeights[y] = 0
      for (var x = 0; x < colGraph.length; x++) {
        ...
        if (col.noSpaceColumn) {
          ...
          colWidths[x] = {
            spacer: spacer.horizontal * (col.e - col.s)
          }
        } else {
          colWidths[x] = 0
        }
  ...
  initializeRowSpacers(lines, colGraph, grid, rowHeights, spacer) {
  ...
  fillGridPosition(line, spacer, params, col, grid, x, y, layerObjectMap, shapesToFill, pathAngle) {
    if (!col.emptySpace) {
      if (line.trim().length === 0) {
        grid[y][x] = {
          object: null,
          width: 0,
          height: spacer.vertical
        }
      ...
      if (la.length > 1) {
        throw new Error('May not have more than one object per row in a column\n ' + line)
      }
      if (/^[- ]+$/.test(la[0])) {
        la[0] = '-'
      } else {
      ...
      if (la[0] === '|'
        ...
        if (type.type === 'shape') {
          ...
          if (type.width === 'fill' || type.height === 'fill') {
            ...
            type.layout = this.layoutShapeWithParams(type.shape, type.params['shape'], null, null, pathAngle)
            if (type.width == null) {
        ...
        grid[y][x] = {
          object: type,
          gridAlign: this.getGridAlign(type, spacer),
          width: type.width,
          height: type.height
        }
  ...
  getGridAlign(type, spacer) {
    let align = {
      horizontal: spacer['grid-align'],
      vertical: spacer['grid-valign']
    }
  ...
  layoutShapeWithParams(shapeKey, params, width, height, pathAngle) {
    ...
    const lines = []
    for (var line of shapeLayout.lines) {
      lines.push(line)
    }

    const shapeParser = new ShapeParser(this.diagramParser)
    const layout = shapeParser.parseLayout(lines, paramsFilled, shapeLayout.spacer, width, height, pathAngle)
    return layout
  }
  ...
  distributeHeights(grid, rows, cols, rowHeights, fillToHeight, userFillToHeight) {
    if (fillToHeight == null) {
      ...
      if (typeof userFillToHeight === 'number') {
        ...
        if (userFillToHeight === '$height') {
          ...
          throw new Error(`Invalid fillToHeight: ${userFillToHeight}`)
        }
      }
    }
        ...
        if (grid[y][x] !== undefined && grid[y][x].object !== undefined) {
          ...
          for (var s = 0; s < grid[y][x].rowspan; s++) {
            ...
            if (rowHeights[rowI].spacer != null) {
              remainingRows++
              takenSoFar += rowHeights[rowI].spacer
          ...
          if (grid[y][x].height === 'fill') {
            let fillHeight = Math.max(fillRemainingHeight, takenSoFar)
          ...
          if (remainingRows === 0) {
            ...
            // Spread across flexible (zeroed) rows
            let heightPerRow = (grid[y][x].height - takenSoFar) / remainingRows
            for (var s = 0; s < grid[y][x].rowspan; s++) {
              const rowI = y + s
              if (rowHeights[rowI].spacer != null) {
                rowHeights[rowI] = rowHeights[rowI].spacer + heightPerRow
              } else if (rowHeights[rowI] == 0) {
                rowHeights[rowI] = heightPerRow
              }
            }
  ...
  distributeWidths(grid, rows, cols, colWidths, pathDistance, fillToWidth, userFillToWidth) {
    if (fillToWidth == null) {
      ...
      fillToWidth = 0
      if (pathDistance > 0 && userFillToWidth == null) {
        fillToWidth = pathDistance
      }
      ...
      if (typeof userFillToWidth === 'number') {
        ...
        if (userFillToWidth === '$width') {
          ...
          throw new Error(`Invalid fillToWidth: ${userFillToWidth}`)
        }
      }
    }
        ...
        if (grid[y][x] !== undefined && grid[y][x].object !== undefined) {
          ...
          for (var s = 0; s < grid[y][x].colspan; s++) {
            ...
            if (colWidths[colI].spacer != null) {
              remainingCols++
              takenSoFar += colWidths[colI].spacer
          ...
          if (grid[y][x].width === 'fill') {
            let fillWidth = Math.max(fillRemainingWidth, takenSoFar)
          ...
          if (remainingCols === 0) {
            ...
            // Spread across flexible (zeroed) cols
            let widthPerColumn = (grid[y][x].width - takenSoFar) / remainingCols
            for (var s = 0; s < grid[y][x].colspan; s++) {
              const colI = x + s
              if (colWidths[colI].spacer != null) {
                colWidths[colI] = colWidths[colI].spacer + widthPerColumn
              } else if (colWidths[colI] == 0) {
                colWidths[colI] = widthPerColumn
              }
            }
  ...
  assertNonZeroWidthAndHeight(colWidths, rowHeights) {
    let width = 0
...
function parseOutSvgFillRotate(line) {
  let svg = null
  let rotateTo = null
  let fillTo = {
    width: null,
  ...
  if (match != null) {
    ...
    if (!fillTo.width.startsWith('$')) {
      ...
      if (isNaN(fillTo.width)) {
        throw new Error(`Invalid fillToWidth: ${match[2]}`)
      }
    }
  }
  ...
  if (match != null) {
    ...
    if (!fillTo.height.startsWith('$')) {
      ...
      if (isNaN(fillTo.height)) {
        throw new Error(`Invalid fillToHeight: ${match[2]}`)
      }
    }
  }

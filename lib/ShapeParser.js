...
module.exports = class ShapeParser {
  ...
  parseLayout(inputFile, params = {}, settings = this.settings, width = null, height = null, parentPathAngle = 0) {
    let line = inputFile.pop()
    let result = this.parserLayout(line, inputFile, params, settings, width, height, parentPathAngle)
  ...
  parserLayout(line, inputFile, params, settings, fillToWidth = null, fillToHeight = null, parentPathAngle = 0) {
    const layersToParse = this.parseLayoutLines(line, inputFile)
  ...
  parseLayoutLines(line, inputFile) {
    const layersToParse = []

    if (inputFile.lines === undefined)
      throw new Error('hans')
  ...
  makeGrid(lines, colGraph, params = {}, settings, pathDistance, pathAngle, fillTo, userFillTo) {
    const grid = [] // [rows][columns]

    // [type] = [array of instances for this type containing x,y coordinate positions]
    const layerObjectMap = {}

    const {rowHeights, colWidths} = this.initializeColumnAndRowArraysWithSpacers(lines, colGraph, settings)
    ...
    this.initializeRowSpacers(lines, colGraph, grid, rowHeights, settings)
  ...
  fillGridPosition(line, settings, params, col, grid, x, y, layerObjectMap, shapesToFill, pathAngle) {
    if (!col.emptySpace) {
  ...
  layoutShapeWithParams(shapeKey, params, width, height, pathAngle) {
    ...
    const inputFile = new NumberedLines(lines.join('\n'))

    const shapeParser = new ShapeParser(this.diagramParser)
    const layout = shapeParser.parseLayout(inputFile, paramsFilled, shapeLayout.settings, width, height, pathAngle)
    return layout
  }

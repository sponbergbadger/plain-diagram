...
module.exports = class ShapeParser {
  ...
  addLayer(layoutLines, layerInfo, params, spacer, pathDistance, pathAngle, fillTo) {
    ...
    // Need to get the right one for the layer, eg. layer 0 of diagram has a margin
    let margin
    if (layer.index === 0) {
  ...
  getLayerInfoUsingFromTo(line, params) {
    ...
    let fromRef =
      this.resolveAnchorRef(match[4])
    let toRef =
      this.resolveAnchorRef(match[10])
  ...
  makeGrid(lines, colGraph, params = {}, spacer, pathDistance, pathAngle, fillTo, userFillTo) {
    ...
    let firstColumnIsEmpty = true
    for (var y = 0; y < lines.length; y++) {
      if (grid[y][0] !== undefined) {

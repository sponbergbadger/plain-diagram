...
module.exports = class LayoutProducer {
    ...
    for (var y = 0; y < rowHeights.length; y++) {
      ...
      for (var x = 0; x < colWidths.length; x++) {
        ...
        if (obj !== undefined && obj.object !== undefined) {
          ...
          this.layoutObject(obj, colX, rowY, cWidth, rHeight)
        }
        colX += colWidth
      }
      rowY += rowHeight
  ...
  getLocationAndSize(layer, margin) {
    ...
    if (layer.layerLocation == null) {
      ...
      if (maker == null) {
        throw new Error(`Unknown location type: ${layer.layerLocation.type}`)
      }
  ...
  getLayerSize(layer, asProjectedIntoImage, angleInDegrees) {
    ...
    if (asProjectedIntoImage) {
      if (layer.colStart === undefined
          || layer.rowStart === undefined) {
  ...
  layoutObject(spec, colX, rowY, colWidth, rowHeight) {
    if (spec.object == null) {
      // spacer
      return
    }
    ...
    if (producer === undefined) {
      throw new Error(`Unknown producer ${spec.object.type}`)
    }
    ...
    const sizeAndPosition = producer(spec.object, position)
    // sizeAndPosition will have xy, xy, width, height, and some elements

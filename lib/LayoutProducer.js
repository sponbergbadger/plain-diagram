...
module.exports = class LayoutProducer {
  ...
  pathDistance(layerInfo) {
    ...
    if (spec != null && spec.from != null) {
      ...
      if (spec.mode === 'box') {
        ...
        return {
          width: this.distance(fromX, fromY, toX, toY),
          height: 0
        }
      }
    } else {
  ...
  layoutLayer(layer, margin, layerIndex, pathAngle) {
    ...
    const locationAndSize = this.getLocationAndSize(layer, margin)
    const {x: colStart, y: rowStart, rotate} = locationAndSize
    this.layerTransformations[layer.zIndex] = {
      rotate
    }

    let box = null
    if (layer.layerLocation != null && layer.layerLocation.mode === 'box') {
      box = {
        from: {
          handleV: layer.layerLocation.from.anchorVAlign,
          handleH: layer.layerLocation.from.anchorHAlign,
          x: locationAndSize.fromX,
          y: locationAndSize.fromY,
        },
        to: {
          handleV: layer.layerLocation.to.anchorVAlign,
          handleH: layer.layerLocation.to.anchorHAlign,
          x: locationAndSize.toX,
          y: locationAndSize.toY,
        }
      }
    }
    ...
    for (var y = 0; y < rowHeights.length; y++) {
      ...
      for (var x = 0; x < colWidths.length; x++) {
        ...
        if (obj !== undefined && obj.object != null) {
          ...
          this.layoutObject(obj, colX, rowY, cWidth, rHeight, gridAlign, box)
        }
        colX += colWidth
      }
      rowY += rowHeight
  ...
  layoutObject(spec, colX, rowY, colWidth, rowHeight, gridAlign, box) {
    if (spec.object == null) {
      // spacer
      return
    }
    ...
    if (producer !== undefined) {
      const position = {
        colX, rowY, colWidth, rowHeight, box
      }
  ...
  locationMakerPath(location, layer) {
    ...
    let rotateAngle = 0
    if (spec.mode === 'path') {
      rotateAngle = this.angle(fromX, fromY, toX, toY)
    }
    ...
    if (spec.mode === 'path') {
      ...
      location.width = origWidth
      location.height = origHeight
      location.fromX = fromX - location.x
      location.fromY = fromY - location.y
      location.toX = toX - location.x
      location.toY = toY - location.y
    }
  }
